...
namespace MavLink
{
    ...
    public static class MavLinkSerializer
    {
        ...
        {
			{220, new MavPacketInfo(Deserialize_TEST_CMD, 37)},
			{221, new MavPacketInfo(Deserialize_TEST_RESP1, 177)},
			{222, new MavPacketInfo(Deserialize_SET_VELOCITY, 148)},
		...
		internal static MavlinkMessage Deserialize_TEST_CMD(byte[] bytes, int offset)
		{
			return new Msg_test_cmd
			{
				arg2 = bitconverter.ToUInt16(bytes, offset + 0),
				cmd_1 = bytes[offset + 2],
				arg1 = bytes[offset + 3],
			};
		}

		internal static MavlinkMessage Deserialize_TEST_RESP1(byte[] bytes, int offset)
		{
			return new Msg_test_resp1
			{
		...
		internal static MavlinkMessage Deserialize_SET_VELOCITY(byte[] bytes, int offset)
		{
			...
			{
				ang_vel = bitconverter.ToUInt16(bytes, offset + 0),
				set_Velocity = bytes[offset + 2],
			};
		}
		...
		internal static MavlinkMessage Deserialize_FOLLOW_TARGET(byte[] bytes, int offset)
		{
			return new Msg_follow_target
			{
				...
				acc =  ByteArrayUtil.ToSingle(bytes, offset + 31, 3),
				attitude_q =  ByteArrayUtil.ToSingle(bytes, offset + 34, 4),
				rates =  ByteArrayUtil.ToSingle(bytes, offset + 38, 3),
				position_cov =  ByteArrayUtil.ToSingle(bytes, offset + 41, 3),
				est_capabilities = bytes[offset + 44],
			};
		}
		...
		internal static int Serialize_TEST_CMD(this Msg_test_cmd msg, byte[] bytes, ref int offset)
		{
			bitconverter.GetBytes(msg.arg2, bytes, offset + 0);
			bytes[offset + 2] = msg.cmd_1;
			bytes[offset + 3] = msg.arg1;
			offset += 4;
			return 220;
		}

		internal static int Serialize_TEST_RESP1(this Msg_test_resp1 msg, byte[] bytes, ref int offset)
		{
		...
		{
			bitconverter.GetBytes(msg.ang_vel, bytes, offset + 0);
			bytes[offset + 2] = msg.set_Velocity;
			offset += 3;
			return 222;
		}
